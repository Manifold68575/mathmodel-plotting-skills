...
def make_figure(output_stem):
    configure()
    x = matrix_demo(160, 8, 941)
    r, p = correlation_pair(x)
    u, s, v = np.linalg.svd(x, full_matrices=False)
    score = u[:, :4] * s[:4]
    fig, ax = plt.subplots(figsize=(7.5, 6))
    fig.subplots_adjust(left=0.15, right=0.96, bottom=0.13, top=0.92)
    cmap = LinearSegmentedColormap.from_list(
        "association", ["#194f65", "#eff7e7", "#b1d5a2"]
    )
    m = coefficient_cells(ax, r, p, mode="lower", cmap=cmap, glyph=True, text=False)
    nodes = np.array([[3, -1.2], [5, 0.6], [7, 2.4], [9, 4.2]])
    for a, end in enumerate(nodes):
        ax.scatter(*end, s=16, c="#a82c76", zorder=5)
        ax.text(end[0] + 0.16, end[1], f"PC{a + 1}", fontsize=7)
        for j in range(8):
            value = np.corrcoef(x[:, j], score[:, a])[0, 1]
            start = [j, j - 0.55]
            curve_edge(
                ax,
                start,
                ...
                "#389989",
                lw=0.25 + 1.2 * abs(value),
                alpha=0.18 + 0.4 * abs(value),
                bend=-0.16,
            )
            ax.scatter(*start, s=6, c="#747474", zorder=4)
    ax.set(xlim=(-2, 10.2), ylim=(8, -2))
    cb = fig.colorbar(m, ax=ax, fraction=0.022, pad=0.015, shrink=0.6, location="left")
    cb.set_label("Pearson's r", fontsize=7)
    handles = [
        Line2D([], [], c="#389989", lw=w, label=l)
        for w, l in zip([0.4, 0.9, 1.4], ["< 0.3", "0.3–0.6", "> 0.6"])
    ]
    ax.legend(
        handles=handles,
...
if __name__ == "__main__":
    make_figure(ROOT / "outputs" / "pca_association_network_replica")
