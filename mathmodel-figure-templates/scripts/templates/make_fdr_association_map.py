...
def make_figure(output_stem):
    configure()
    x = matrix_demo(160, 13, 872)
    r, p = correlation_pair(x)
    q = np.ones_like(p)
    mask = np.tril_indices(13, -1)
    q[mask] = adjust_fdr(p[mask])
    q[(mask[1], mask[0])] = q[mask]
    fig, ax = plt.subplots(figsize=(8, 7.5))
    fig.subplots_adjust(left=0.12, right=0.84, bottom=0.15, top=0.91)
    m = coefficient_cells(ax, r, q, mode="lower", cmap="RdBu_r", text=True)
    for i in range(13):
        ax.text(i, i, f"X{i + 1}", ha="center", va="center", fontsize=6)
    ax.text(
        7,
        3,
        "FDR-adjusted q\n* < 0.05\n** < 0.01\n*** < 0.001",
        fontsize=7,
        bbox=dict(fc="white", ec=".45", lw=0.5, pad=5),
    )
    fig.colorbar(m, cax=fig.add_axes([0.91, 0.23, 0.025, 0.62]), label="Pearson r")
    export(fig, output_stem)


if __name__ == "__main__":
    make_figure(ROOT / "outputs" / "fdr_association_map_replica")
