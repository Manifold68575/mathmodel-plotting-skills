...
def make_figure(output_stem):
    ...
    fig.subplots_adjust(left=0.08, right=0.92, top=0.97, bottom=0.13)
    ax.set_theta_offset(np.pi / 2)
    ax.set_theta_direction(-1)
    cmap = mpl.colormaps["autumn"]
    norm = Normalize(-1, 1)
    for g in range(4):
        r, p = correlation_pair(matrix_demo(100, n, 837 + g))
        start = g * np.pi / 2 + 0.08
        span = np.pi / 2 - 0.16
        for row in range(n - 1):
            count = n - row - 1
            radius = 1.0 + (n - row - 2) * 0.12
            for j in range(count):
                angle = start + (j + 0.5) * span / count
                ax.bar(
                    angle,
                    0.114,
                    bottom=radius,
                    width=span / count * 0.92,
                    color=cmap(norm(r[row, row + j + 1])),
                    edgecolor="white",
                    lw=0.25,
                )
                if p[row, row + j + 1] < 0.01:
                    ax.text(
                        angle,
                        radius + 0.055,
                        "*",
        ...
        ax.text(
            start + span / 2,
            3.23,
            ["Humid", "Semi-humid", "Semi-arid", "Arid"][g],
            ha="center",
            fontsize=7,
        )
    ax.set_ylim(0, 3.4)
    ...
    fig.colorbar(
        ...
        cax=fig.add_axes([0.15, 0.065, 0.7, 0.015]),
...
if __name__ == "__main__":
    make_figure(ROOT / "outputs" / "petal_correlation_panels_replica")
