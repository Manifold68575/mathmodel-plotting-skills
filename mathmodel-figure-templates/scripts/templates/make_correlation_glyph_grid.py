...
def make_figure(output_stem):
    ...
    latent = rng.normal(size=130)
    x = latent[:, None] * rng.choice([-1, 1], 16) + rng.normal(0, 0.45, (130, 16))
    r, p = correlation_pair(x)
    n = len(r)
    fig, ax = plt.subplots(figsize=(8, 7))
    fig.subplots_adjust(left=0.12, right=0.86, top=0.92, bottom=0.16)
            ...
            if i > j:
                ax.add_patch(
                    Ellipse(
                        (j, i),
                        0.85,
                        0.12 + 0.75 * (1 - abs(r[i, j])),
                        angle=45 if r[i, j] >= 0 else -45,
                        fc=cmap(norm(r[i, j])),
                        ec="none",
                    )
                )
    ...
    ax.set(xlim=(-0.6, n - 0.4), ylim=(n - 0.4, -0.6), aspect="equal")
    ax.set_xticks(range(n), [f"V{i + 1}" for i in range(n)], rotation=90, fontsize=6)
    ax.set_yticks(range(n), [f"V{i + 1}" for i in range(n)], fontsize=6)
    ax.tick_params(length=0)
    ax.spines[:].set_visible(False)
    fig.colorbar(
        mpl.cm.ScalarMappable(norm=norm, cmap=cmap),
        cax=fig.add_axes([0.91, 0.20, 0.022, 0.63]),
        label="Pearson r",
    )
...
if __name__ == "__main__":
    make_figure(ROOT / "outputs" / "correlation_glyph_grid_replica")
