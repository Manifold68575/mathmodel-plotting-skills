...
def make_figure(output_stem):
    ...
    x = rng.normal(size=(140, 11))
    x[:, 1] = x[:, 0] + rng.normal(0, 0.22, 140)
    ...
    m = coefficient_cells(ax, r, p, mode="lower", cmap="seismic", text=True)
    for i in range(11):
        ax.add_patch(Rectangle((i - 0.5, i - 0.5), 1, 1, fc=".05"))
        ax.text(i, i, "1.00", ha="center", va="center", color="white", fontsize=5)
    end = (12, 2)
    for i in range(11):
        curve_edge(
            ax,
            ...
            "#c25e55" if r[i, 0] > 0 else "#4f7ea1",
            lw=0.6 + abs(r[i, 0]),
            alpha=0.7,
            bend=-0.15,
        )
    ax.scatter(*end, s=15, c=".2")
    ax.text(12.2, 2, "Target", fontsize=8)
    ax.set(xlim=(-0.5, 14), ylim=(10.5, -1))
    ax.set_title("(a) Correlation network", loc="left")
    polar = fig.add_subplot(gs[1], projection="polar")
    vif = np.diag(np.linalg.pinv(r))
    theta = np.linspace(0, 2 * np.pi, 11, endpoint=False)
    palette = mpl.colormaps["hsv"](np.linspace(0, 0.85, 11))
    polar.bar(theta, vif, width=0.35, color=palette, edgecolor="white", lw=0.3)
    polar.set_xticks(theta, [f"X{i + 1}" for i in range(11)], fontsize=6)
    polar.tick_params(labelsize=5)
    polar.grid(lw=0.4, alpha=0.4)
    polar.set_title("(b) Variance inflation factors", pad=20)
    fig.colorbar(
        m,
        cax=fig.add_axes([0.13, 0.075, 0.34, 0.018]),
...
if __name__ == "__main__":
    make_figure(ROOT / "outputs" / "network_profile_report_replica")
